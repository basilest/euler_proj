...
my ($max_H, $max_V, $max_D1, $max_D2) = (0,0,0,0);
my ($max_H_seq, $max_V_seq, $max_D1_seq, $max_D2_seq);
my ($max_H_i, $max_V_i, $max_D1_i, $max_D2_i);
my $n;
my $i;
my $S=2;
...
sub max_H {
    ...
    for ($r=0; $r<20; $r++) {
        for ($c=0; $c<17; $c++) {
            my $a = $A[$r];
            my $prod = $a->[$c]*$a->[$c+1]*$a->[$c+2]*$a->[$c+3];
            if ($prod > $max_H ) {
                $max_H = $prod; 
                $max_H_seq = "$a->[$c] - $a->[$c+1] - $a->[$c+2] - $a->[$c+3]";
                $max_H_i = $r;
            }
        }
    }
...
sub max_V {
    ...
    for ($c=0; $c<20; $c++) {
        for ($r=0; $r<17; $r++) {
            my $prod = $A[$r][$c]*$A[$r+1][$c]*$A[$r+2][$c]*$A[$r+3][$c];
            if ($prod > $max_V ) {
                $max_V = $prod; 
                $max_V_seq = "$A[$r][$c] - $A[$r+1][$c] - $A[$r+2][$c] - $A[$r+3][$c]";
                $max_V_i = $c;
            }
        }
    }
...
sub max_D1 {
    ...
    for ($c=0; $c<17; $c++) {
        for ($r=0; $r<20; $r++) {
            my $prod = $A[$r][$c]*$A[$r+1][$c+1]*$A[$r+2][$c+2]*$A[$r+3][$c+3];
            if ($prod > $max_D1 ) {
                $max_D1 = $prod; 
                $max_D1_seq = "$A[$r][$c] - $A[$r+1][$c+1] - $A[$r+2][$c+2] - $A[$r+3][$c+3]";
                $max_D1_i = $c ."-". $r;
            }
        }
    }
...
sub max_D2 {
    ...
    for ($c=3; $c<20; $c++) {
        for ($r=0; $r<20; $r++) {
            ...
            if ($prod > $max_D2 ) {
                $max_D2 = $prod; 
                $max_D2_seq = "$A[$r][$c] - $A[$r+1][$c-1] - $A[$r+2][$c-2] - $A[$r+3][$c-3]";
                $max_D2_i = $c ."-". $r;
            }
        }
    }
...
max_H();
print "The H max is $max_H (row $max_H_i:$max_H_seq)\n";
max_V();
print "The V max is $max_V (col $max_V_i:$max_V_seq)\n";
max_D1();
print "The D1 max is $max_D1 (col $max_D1_i:$max_D1_seq)\n";
max_D2();
print "The D2 max is $max_D2 (col $max_D2_i:$max_D2_seq)\n";
